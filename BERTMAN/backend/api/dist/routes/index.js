...
let quartieriGeometryCache = null;
let poiGeometryCache = null;
function loadQuartieriGeometry() {
    if (!quartieriGeometryCache) {
        try {
            const data = fs_1.default.readFileSync(quartieriJsonPath, "utf8");
            const geojson = JSON.parse(data);
            // Extract quartieri from the GeoJSON FeatureCollection
            quartieriGeometryCache = geojson.features
                .filter((feature) => feature.properties.entity_type === "quartiere")
                .map((feature) => ({
            ...
            poiGeometryCache = geojson.features
                .filter((feature) => feature.properties.entity_type === "point")
                .map((feature) => ({
                tipo_poi: feature.properties.tipo_poi,
                quartiere_id: feature.properties.quartiere_id,
                geometry: feature.geometry
            }));
            console.log(`Loaded quartieri_poi.geojson into cache: ${quartieriGeometryCache.length} quartieri, ${poiGeometryCache.length} POI`);
        }
        catch (err) {
...
const getDb = () => {
    return new sqlite3_1.default.Database(dbPath, (err) => {
        if (err) {
...
router.get("/get-data", (req, res) => {
    ...
    let crimes = req.query.crimes || "";
    let quartieri = req.query.quartieri || "";
    ...
    if (!quartieri ||
        ...
        const invalidQuartieri = quartieriList.filter((q) => !validQuartieri.includes(q));
    ...
    db.all(query, params, (err, rows) => {
        ...
        try {
            ...
            quartieriList.forEach((quartiere) => {
                const matching_quartiere = geometry_json.find((feature) => feature.python_id === quartiere);
            ...
            // Get POI filtered by selected quartieri (calculate BEFORE analysis)
            const poi_json = loadPOIGeometry();
            const filtered_poi = poi_json.filter((poi) => quartieriList.includes(poi.quartiere_id));
            // Calculate POI counts per quartiere (needed for S_poi sub-index)
            const poiCountsByQuartiere = {};
            filtered_poi.forEach((poi) => {
                ...
                if (poi.tipo_poi in quartierePoiCounts &&
                    quartierePoiCounts[poi.tipo_poi] !== undefined) {
                    quartierePoiCounts[poi.tipo_poi]++;
                }
            });
            // Analysis with new CRI formula
            const analyzed_geojson = (0, utils_1.analyze_quartieri)(articles_df, quartieri_data, geojson_data, crimesList, poiCountsByQuartiere, {
                enableCrimeSubIndex: true,
                enablePoiSubIndex: true,
                enableSocioEconomicSubIndex: false,
                enableEventSubIndex: false
            });
            // Statistics
            const final_geojson = (0, utils_1.calculate_statistics)(quartieri_data, analyzed_geojson);
            // Add POI counts to each feature's properties
            final_geojson.features.forEach((feature) => {
                feature.properties.poi_counts = poiCountsByQuartiere[feature.properties.python_id] || {
                    bar: 0,
                    scommesse: 0,
                    bancomat: 0,
                    stazione: 0
...
router.get("/get-articles", (req, res) => {
    const quartiere = req.query.quartiere;
...
router.post("/upload-to-database", (req, res) => {
  ...
  `);
    db.serialize(() => {
        jsonFile.forEach((item) => {
            stmt.run(item.link || "", item.python_id || "", item.title || "", item.date || "", item.content || "", item.omicidio?.value, item.omicidio?.prob, item.omicidio_colposo?.value, item.omicidio_colposo?.prob, item.omicidio_stradale?.value, item.omicidio_stradale?.prob, item.tentato_omicidio?.value, item.tentato_omicidio?.prob, item.furto?.value, item.furto?.prob, item.rapina?.value, item.rapina?.prob, item.violenza_sessuale?.value, item.violenza_sessuale?.prob, item.aggressione?.value, item.aggressione?.prob, item.spaccio?.value, item.spaccio?.prob, item.truffa?.value, item.truffa?.prob, item.estorsione?.value, item.estorsione?.prob, item.contrabbando?.value, item.contrabbando?.prob, item.associazione_di_tipo_mafioso?.value, item.associazione_di_tipo_mafioso?.prob);
        });
        stmt.finalize();
    });
    db.close((err) => {
